 ...
 * Time: 9:01:39 PM
 */
public class ThrustOutsideBarPattern implements IIndicator {
    ...
    public void onStart(IIndicatorContext context) {
        indicatorInfo =
                new IndicatorInfo("THRUSTOUTSIDEBAR", "Thrust Outside Bar", "Pattern Recognition", true, false, false, 1, 0, 6);
    ...
    public IndicatorResult calculate(int startIndex, int endIndex) {
        ...
        for (i = startIndex, j = 0; i <= endIndex; i++, j++) {
            ...
            // bullish
            if ((inputs[0][1][i] > inputs[0][2][i - 1]) && ((inputs[0][3][i] < inputs[0][3][i - 1]))) {
                isBull = true;
            }
            // bearsish
            boolean isBear = false;
            if ((inputs[0][1][i] < inputs[0][3][i - 1]) && ((inputs[0][2][i] > inputs[0][2][i - 1]))) {
                isBear = true;
            }
