...
//	------------------------------------------------------ ABSTRACT METHODS -------------------------------------------------------
	...
	public abstract boolean checkEmergingPatternPostTrend(double[][] sourceData, int dirtyPatterStartIndex, int dirtyPatternSize, IndexValue[] patternPoints, IndexValue[] upperAsymptotePoints, IndexValue[] bottomAsymptotePoints);
...
//	------------------------------------------------------ INDICATOR METHODS -------------------------------------------------------
	...
	protected double getPatternPriceSpread(double[] cleanPattern) {
...
//	------------------------------------------------------ MATH UTILS -------------------------------------------------------
	...
	public static double getPatternSpread(double[] cleanPattern) {
	    double max = cleanPattern[0];
	    double min = cleanPattern[0];
	    for (int i = 1; i < cleanPattern.length; i++) {
	        max = Math.max(max, cleanPattern[i]);
...
//	------------------------------------------------------ HELPER METHODS -------------------------------------------------------
	...
	protected boolean checkCandlePostTrendIsUnderUpperAsymptote(
	    ...
	    for (int i = patternEndIdx;  i < sourceData[0].length;  i++, asymptoteValue += asymptoteGrowthStep) {
	        if (sourceData[highIndex][i] > asymptoteValue) {
	            return false;
	        }
	    }
	...
	protected boolean checkCandlePostTrendIsOverBottomAsymptote(
        ...
        for (int i = patternEndIdx;  i < sourceData[0].length;  i++, asymptoteValue += asymptoteGrowthStep) {
            if (sourceData[lowIndex][i] < asymptoteValue) {
                return false;
            }
        }
